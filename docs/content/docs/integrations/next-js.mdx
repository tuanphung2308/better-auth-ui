...
## Account and Organization Pages (App Router)

Add dedicated routes for account and organization views.

### Account pages

```tsx title="app/account/[pathname]/page.tsx"
...
```tsx title="app/organization/[pathname]/page.tsx"
import { organizationViewPaths } from "@daveyplate/better-auth-ui/server"
import { OrganizationView } from "@daveyplate/better-auth-ui"

export function generateStaticParams() {
    return Object.values(organizationViewPaths).map((pathname) => ({ pathname }))
}

export default async function OrganizationPage({ params }: { params: Promise<{ pathname: string }> }) {
    const { pathname } = await params
    return (
        <main className="container p-4 md:p-6">
...
If you prefer slug-based org URLs, set `organization={{ slugPaths: true, basePath: "/organization" }}` in the provider and structure your routes accordingly (e.g. `app/organization/[slug]/[pathname]/page.tsx`).