...
In order to use `@daveyplate/better-auth-ui` with the `@daveyplate/better-auth-tanstack` package, all you need to do is change your [`<AuthUIProvider>`](/components/auth-ui-provider) into an `<AuthUIProviderTanstack>` component. It accepts all of the same props and configuration options.
