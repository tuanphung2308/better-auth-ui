...
To enable avatar support, you need to configure the `avatar` and optionally `uploadAvatar` properties within your [`AuthUIProvider`](../components/auth-ui-provider):
...
- **File Format**: You can customize the uploaded image file format via `avatarExtension` prop in [`AuthUIProvider`](../components/auth-ui-provider). It defaults to `"png"`.
